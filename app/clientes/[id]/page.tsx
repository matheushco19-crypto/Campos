...
export default async function ClientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="content client-page">
    ...
    <section className="panel"><div className="panel-head"><div><h2>Contas e instituições</h2><p className="muted">Relacionamentos detectados a partir dos documentos importados.</p></div></div><div className="client-grid">{(accounts ?? []).map(a => <div className="client-card" key={a.id}><div className="client-avatar">{a.institution_id === 'nubank' ? 'N' : 'B'}</div><div><strong>{a.institution_id === 'nubank' ? 'Nubank' : a.institution_id}</strong><span>{a.account_type} · {a.masked_identifier || 'identificador protegido'}</span></div></div>)}</div></section>
    <section className="panel"><div className="panel-head"><div><h2>Documentos</h2><p className="muted">Fluxo de importação e integridade.</p></div><span className="counter">{documents?.length ?? 0}</span></div><div className="table-wrap"><table><thead><tr><th>Documento</th><th>Período</th><th>Processamento</th><th>Integridade</th><th>Semântica</th></tr></thead><tbody>{(documents ?? []).map(d => <tr key={d.id}><td><strong>{d.file_name}</strong><small>{d.document_type}</small></td><td>{d.period_start && d.period_end ? `${new Date(d.period_start).toLocaleDateString('pt-BR')} – ${new Date(d.period_end).toLocaleDateString('pt-BR')}` : '—'}</td><td><span className={`pill ${d.processing_status}`}>{d.processing_status}</span></td><td><span className={`pill ${d.data_integrity_status}`}>{d.data_integrity_status}</span></td><td><span className={`pill ${d.semantic_classification_status}`}>{d.semantic_classification_status}</span></td></tr>)}</tbody></table></div></section>
    <section className="grid-2"><div className="panel"><div className="panel-head"><div><h2>Movimentações</h2><p className="muted">Lançamentos ativos mais recentes.</p></div><span className="counter">{transactions?.length ?? 0}</span></div><div className="table-wrap"><table><thead><tr><th>Data</th><th>Descrição</th><th>Valor</th><th>Tipo</th><th>Conf.</th></tr></thead><tbody>{(transactions ?? []).map(t => { const neutral=t.is_transfer||t.is_investment_flow||t.is_card_payment; return <tr key={t.id}><td>{new Date(t.transaction_date).toLocaleDateString('pt-BR')}</td><td><strong>{t.description_original}</strong><small>{t.is_investment_flow ? 'fluxo de investimento' : t.is_card_payment ? 'pagamento de cartão' : t.is_transfer ? 'transferência' : t.expense_nature || '—'}</small></td><td className={neutral ? '' : t.direction === 'credit' ? 'positive' : 'negative'}>{neutral ? '' : t.direction === 'credit' ? '+' : '-'}{brl.format(Number(t.amount))}</td><td>{neutral ? 'Neutro' : t.direction === 'credit' ? 'Entrada' : 'Saída'}</td><td>{Math.round(Number(t.classification_confidence || 0) * 100)}%</td></tr>})}</tbody></table></div></div><div className="panel"><div className="panel-head"><div><h2>Pendências</h2><p className="muted">Validações que exigem atenção.</p></div></div>{(reviews ?? []).length === 0 ? <div className="empty"><div className="empty-icon">✓</div><strong>Nenhuma pendência aberta</strong><span>O dossiê está operacionalmente limpo.</span></div> : <div className="review-list">{reviews?.map(r => <div className="review-item" key={r.id}><span className={`severity ${r.severity}`}>{r.severity}</span><div><strong>{r.reason}</strong><small>{new Date(r.created_at).toLocaleString('pt-BR')}</small></div></div>)}</div>}</div></section>
  </main>
}
