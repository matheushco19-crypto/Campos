...
export default async function ClientsPage() {
  ...
  const [{ data: clients }, { data: consultants }] = await Promise.all([
    supabase
      .from('clients')
      .select('id,full_name,email,cpf_cnpj,client_code,fee_type,fee_value,consultant_id,status,created_at')
      .eq('status', 'active')
      .order('full_name'),
    supabase
      .from('users')
  ...
  return (
    <main className="app-shell">
      ...
      <section className="content">
        <ClientPageStyles />
        <ClientList initialClients={clients ?? []} consultants={consultants ?? []} />
      </section>
    </main>
  )
}
